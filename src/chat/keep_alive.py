...
class KeepAlive:
    ...
    async def wake_pong(self,
                        msg_id: str):  # deve ser implementado para quando for recebido um pong, de forma a acordar o manage_pong
    ...
    async def manage_pong(self, msg_id, peer_id):
        ...
        try:
            await asyncio.wait_for(event.wait(),
                                   timeout=10.0)  # fica "de olho" no evento esperando o sinal por até x segundos

            rtt = (time.monotonic() - start_time) * 1000
            self.peer_table.update_rtt(peer_id, rtt)  # SINCRONIZAÇÃO: atualiza a PeerTable global
            self.peer_server.connections[
                peer_id].rtt_ms = rtt  # atualiza o rtt em connections de peer_connections também
