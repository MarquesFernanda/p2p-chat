...
class PeerServer:
    ...
    async def accept_new_peer(self, reader: asyncio.StreamReader,
                              writer: asyncio.StreamWriter):  # handles inbound connections
        ...
        try:
            ...
            if data_hello.get('type') == 'HELLO':
                ...
                if not self.peer_table.contains(
                        remote_peer_id):  # verifica se peer está registrado na peertable, se não, o registra via upsert_rdv
    ...
    async def request_connection(self, remote_peer_id: str, ip: str, port: int, timeout: float) -> bool:
        if (remote_peer_id in self.connections) or (
                remote_peer_id in self._dialing):  # evita fazer duplas conexões com mesmo peer
            return True
    ...
    async def _handle_peer_messages(self, remote_peer_id: str, reader: asyncio.StreamReader,
                                    writer: asyncio.StreamWriter):
        try:
            while self._running:
                ...
                if msg.get('type') == 'PING':
                    ...
                    if self.autonomous_mode:
                        ...
                        await self.send_json(writer, auto_reply_msg)
                        self.logger.debug(
                            f"[PeerServer] Resposta automática de PUB enviado em {dst} para {remote_peer_id}")
    ...
    async def send_json(self, writer: asyncio.StreamWriter, msg: dict):
        encoded_msg = (json.dumps(msg, separators=(",", ":")) + "\n").encode(
            "utf-8")  # transforma dicionário de msg em json file termiado em \n
