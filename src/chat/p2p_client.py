...
class p2pChatApp:
    ...
    async def run(self) -> int:
        ...
        try:
            ...
            self._tasks.clear()
            # CORREÇÃO 5 (continuação): shutdown agora é sempre chamado ao sair
            await self.shutdown()
    ...
    async def _discover_and_update(self) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"[P2P] DISCOVER: {len(peers)} peer(s) no namespace '{self.config.namespace}'"
            )
    ...
    async def _reconnect_peers(self) -> None:
        ...
        self.logger.debug(f"[P2P] Tentando reconectar {len(candidatos)} peer(s)...")
